    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl sm:text-2xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2.5">
                <i class="bi bi-chat-left-text-fill text-[#4F1C51]"></i>
                Feedback & Aduan Pengunjung
            </h2>
            <p class="text-xs sm:text-sm text-gray-500 mt-1">
                Kelola pesan, aduan, dan feedback publik yang masuk melalui form website SIPA.
            </p>
        </div>
        ...
        <div class="flex items-center gap-2 bg-gray-100/80 p-1.5 rounded-xl text-xs font-bold shrink-0">
            <a href="{{ route('admin.dashboard.showComplaint') }}" 
               class="px-3.5 py-2 rounded-lg transition-all flex items-center gap-1.5 {{ request()->routeIs('admin.dashboard.showComplaint') ? 'bg-white text-[#4F1C51] shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                <span>Semua</span>
                <span class="px-1.5 py-0.5 rounded-full text-[10px] {{ request()->routeIs('admin.dashboard.showComplaint') ? 'bg-purple-100 text-[#4F1C51]' : 'bg-gray-200 text-gray-700' }}">
                    {{ $totalAll ?? count($complaints) }}
                </span>
            </a>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-5 gap-4 border-b border-gray-100 bg-gray-50/40">
            <div class="flex items-center gap-2">
                <span class="text-xs font-extrabold uppercase tracking-wider text-gray-500 flex items-center gap-1.5">
                    <i class="bi bi-funnel-fill text-[#4F1C51]"></i>
                    Daftar Aduan Masuk
                </span>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($complaints as $index => $complaint)
                        <tr class="hover:bg-purple-50/20 transition-colors group">
                            ...
                            <td class="px-5 py-4 align-top">
                                <div class="flex items-start gap-3">
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-purple-100 to-pink-100 border border-purple-200 text-[#4F1C51] font-extrabold text-xs flex items-center justify-center shrink-0 shadow-sm">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <a href="mailto:{{ $complaint->email }}" class="text-xs text-gray-500 hover:text-[#4F1C51] font-medium transition-colors block truncate mt-0.5" title="{{ $complaint->email }}">
                            ...
                            <td class="px-5 py-4 align-top">
                                <div class="space-y-1.5">
                                    <h5 class="text-xs sm:text-sm font-bold text-[#4F1C51] tracking-tight">
                                        {{ $complaint->subject }}
                                    </h5>
                                    ...
                                    <!-- Quick preview if already replied -->
                        ...
                        <!-- HIDDEN DATA STORE FOR DETAIL MODAL (If Replied) -->
                        @if ($complaint->status === 'sudah dibalas')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-14 text-center">
                                ...
                                <h4 class="text-sm font-bold text-gray-800">Tidak ada aduan / keluhan ditemukan</h4>
                                <p class="text-xs text-gray-500 mt-1 max-w-sm mx-auto">
                                    @if(request('search'))
                                        Tidak ada pesan yang cocok dengan kata kunci "<strong>{{ request('search') }}</strong>".
                                    @else
                                        Belum ada pesan aduan atau feedback yang masuk pada kategori ini.
                                    @endif
                                </p>
                                @if(request('search'))
