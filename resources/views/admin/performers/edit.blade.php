...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
            ...
            <span>/</span>
            <span class="text-gray-900 font-semibold">Edit Performer</span>
        </div>
...
<div class="bg-white rounded-2xl border border-gray-150 shadow-sm p-6 sm:p-8 max-w-4xl">
    <form action="{{ route('admin.performers.update', $performer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type" 
                        ...
                        required
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 text-sm focus:ring-2 focus:ring-[#4F1C51] focus:border-[#4F1C51] bg-gray-50/50 hover:bg-white transition-all">
                    <option value="international" {{ old('type', $performer->type) === 'international' ? 'selected' : '' }}>🌍 International Delegate (Delegasi Internasional)</option>
                    <option value="national" {{ old('type', $performer->type) === 'national' ? 'selected' : '' }}>🇮🇩 National Delegate (Delegasi Nasional)</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" 
                       name="category" 
                       id="category" 
                       value="{{ old('category', $performer->category) }}" 
            ...
            <div>
                ...
                <input type="text" 
                       name="country" 
                       id="country" 
                       value="{{ old('country', $performer->country) }}" 
                       required 
            ...
            <div>
                ...
                <input type="text" 
                       name="country_badge" 
                       id="country_badge" 
                       value="{{ old('country_badge', $performer->country_badge) }}" 
            ...
            <div>
                ...
                <input type="number" 
                       name="order" 
                       id="order" 
                       value="{{ old('order', $performer->order) }}" 
                       class="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 text-sm focus:ring-2 focus:ring-[#4F1C51] focus:border-[#4F1C51] bg-gray-50/50 hover:bg-white transition-all">
            </div>
            ...
            <div class="flex items-center pt-8">
                <label class="relative flex items-center gap-3 cursor-pointer select-none">
                    <input type="checkbox" 
                           ...
                           value="1" 
                           {{ old('is_featured_home', $performer->is_featured_home) ? 'checked' : '' }}
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" 
                          ...
                          class="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 text-sm focus:ring-2 focus:ring-[#4F1C51] focus:border-[#4F1C51] bg-gray-50/50 hover:bg-white transition-all resize-none">{{ old('description', $performer->description) }}</textarea>
            </div>

            <!-- Photo Upload with Automatic WebP Conversion -->
            <div class="md:col-span-2">
                <label class="block text-sm font-bold text-gray-700 mb-1.5">
                    Ganti Foto Performer <span class="text-xs font-normal text-gray-400">(Biarkan kosong jika tidak ingin mengubah foto)</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                    ...
                    <div class="md:col-span-8 border-2 border-dashed border-gray-300 hover:border-[#4F1C51] rounded-2xl p-6 text-center transition-all bg-gray-50/50 hover:bg-[#4F1C51]/5 cursor-pointer relative">
                        ...
                        <div class="flex flex-col items-center justify-center space-y-1.5 pointer-events-none">
                            ...
                            <span class="text-sm font-bold text-[#4F1C51]">Pilih Foto Baru untuk Mengganti</span>
                    ...
                    <div class="md:col-span-4 flex flex-col items-center justify-center">
                        <div class="w-28 h-36 rounded-2xl overflow-hidden bg-gray-100 border-2 border-gray-200 flex items-center justify-center text-gray-400 relative shadow-sm">
                            <img id="imagePreview" src="{{ asset('images/' . $performer->image_path) }}" alt="{{ $performer->name }}" class="w-full h-full object-cover">
                            <span id="previewPlaceholder" class="text-xs text-center px-2 hidden">Preview Foto</span>
                        </div>
                        <span class="text-[11px] text-gray-400 mt-1">Foto Saat Ini</span>
                    </div>
