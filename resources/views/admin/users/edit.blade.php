    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h2 class="text-xl sm:text-2xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2.5">
                <i class="bi bi-person-gear text-[#4F1C51]"></i>
                Edit User & Hak Akses
            </h2>
            <p class="text-xs sm:text-sm text-gray-500 font-medium mt-1">
                Perbarui data akun {{ $user->name }} dan sesuaikan fitur apa saja yang boleh diakses.
            </p>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="role" 
                        ...
                        <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Admin (Dapat disesuaikan fitur)</option>
                        <option value="staff" {{ old('role', $user->role) === 'staff' ? 'selected' : '' }}>Staff Operator (Fitur terbatas)</option>
                        <option value="superadmin" {{ old('role', $user->role) === 'superadmin' ? 'selected' : '' }}>Super Administrator (Akses Penuh)</option>
                    </select>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-150 shadow-sm p-6 sm:p-7 space-y-5" id="permissions-section">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="permission-grid">
                @foreach ($availablePermissions as $key => $perm)
                    @php
                        $isChecked = in_array($key, old('permissions', $userPerms));
                    @endphp
