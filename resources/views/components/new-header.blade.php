...
<nav class="navbar-custom  bg-white shadow-md fixed top-0 left-0 right-0 z-50">
  <div class="container mx-auto px-4">
    ...
    <div id="mobile-menu" class="md:hidden absolute top-full left-0 w-full bg-white shadow-md pb-4 transition-all duration-300 ease-in-out transform origin-top opacity-0 scale-95 pointer-events-none">
      <a href="/" class="block py-2 px-4 text-sm font-bold hover:bg-gray-100 {{ request()->is('/') ? 'nav-link-active' : '' }}">HOME</a>
      <a href="/aboutus/director" class="block py-2 px-4 text-sm hover:bg-gray-100">DIRECTOR PROFILE</a>
      <a href="/aboutus/history" class="block py-2 px-4 text-sm hover:bg-gray-100">HISTORY OF SIPA</a>
      <a href="/lineup" class="block py-2 px-4 text-sm font-bold hover:bg-gray-100 {{ request()->is('lineup') ? 'nav-link-active' : '' }}">LINE UP</a>
      <a href="#" class="block py-2 px-4 text-sm font-bold hover:bg-gray-100">GALLERY</a>
      <a class="block py-2 px-4 text-sm" href="{{ route('lang.switch', app()->getLocale() == 'id' ? 'en' : 'id') }}">
        Switch to {{ app()->getLocale() == 'id' ? 'English' : 'Indonesia' }}
      </a>
    </div>
  </div>
</nav>
